...
### 1. 🧠 에빙하우스 망각 곡선 기반 간격 반복 (Spaced Repetition System, SRS)
- **과학적 복습 스케줄링**: SuperMemo SM-2 및 에빙하우스 망각 곡선 알고리즘을 기반으로 망각 직전 최적의 타이밍에 복습 큐(Due Queue) 자동 생성.
- **숙련도 4단계 관리**: 미학습(New) ➜ 초기 학습(Learning) ➜ 주기 복습(Reviewing) ➜ 완전 암기(Mastered) 단계 자동 전이.
...
### 2. 📄 종이 출력용 맞춤형 A4 PDF 시험지·학습표 생성 (QuestPDF)
- **다양한 시험지 모드**:
...
### 3. 📷 작성한 시험지 사진 자동 채점 (OCR Scanner & Vision)
...
- Tailscale 특유의 딥 차콜(`#131217`), 슬레이트(`#1e1d24`), 인디고(`#6366f1`), 에메랄드(`#10b981`) 정제된 컬러 팔레트.
...
- **언어 및 프레임워크**: C# 12, .NET 8 WPF (`net8.0-windows10.0.19041.0`)
- **아키텍처**: MVVM Pattern (`CommunityToolkit.Mvvm`)
- **PDF 렌더링**: QuestPDF 2026
- **OCR 및 영상 처리**: `Windows.Media.Ocr`, `Windows.Graphics.Imaging`
- **데이터 저장**: System.Text.Json (Local JSON storage in `%AppData%/BenkyoKanji/data`)
- **테스트 프레임워크**: xUnit 2.5, .NET Test SDK

---

## 🚀 시작하기 및 빌드 (Getting Started)
...
```powershell
# 프로젝트 디렉터리로 이동
cd c:\Users\nhc33\Desktop\Develop\benkyo-kanji
...
## 📝 라이선스 & 커밋 규칙
- 본 프로젝트는 커밋마다 `ADDED:`, `FEATURE:`, `FIXED:` 접두어를 사용하며 1줄 이내로 작성됩니다.
